...
@Configuration //스프링 컨테이너에서 빈(bean-객체)으로 생성, 관리 - 필요한 곳에 의존성 주입
public class SecurityConfig {

    //Spring Security 5 버전부터는 비밀번호는 반드시 암호화를 해야함
    //비밀번호를 암호화하지 않으면 HTTP 403(access denied, 접근거부) 또는
    //HTTP 500(internal server error, 내부서버오류)가 발생
    ...
    public UserDetailsService inMemoryUserDetailsService() {
        ...
        UserDetails user1 = User
                .withUsername("user1")  //로그인할때 사용할 사용자 이름
                .password(passwordEncoder().encode("1111")) //로그인할 때 사용할 비밀번호.-> 비밀번호는 반드시 암호화!!encoder사용
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
       ...
       //페이지 접근 권한 설정
        ...
        http.authorizeHttpRequests((authRequest) ->
                ...
                //권한이 필요한 페이지들을 설정
                .requestMatchers("/post/create", "/post/details", "/post/modify", "/post/update", "/post/delete", "/api/reply/**") //순서중요!! 권한 필요한 것 먼저 써줌.
//                .authenticated() admin,user 권한여부에 상관없이 아이디 패스워드가 일치하면 접근가능.
        ...
        */
        ...
        //(2) 각각의 컨트롤러 메서드에서 @PreAuthorize 애너테이션을 사용
